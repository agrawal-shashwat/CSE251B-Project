...
def load_vocab(vqa, ques_ids, threshold):
    if os.path.isfile('savedVocab'):
        ...
        vocab = build_vocab(vqa, ques_ids, threshold)
...
def build_vocab(vqa, ques_ids, threshold):
    counter = Counter()
    for i, id in enumerate(ques_ids):
        caption = str(vqa.qqa[id]['question'])
